    ...
    <div>
      {conversations.map((conversation, index) => (
        <div 
          ...
          className={`flex items-center p-3 border-b border-gray-700 cursor-pointer transition-all hover:bg-gray-700 ${
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex justify-between">
              <h3 className="font-medium text-white truncate blur-sm">{conversation.name}</h3>
              <span className="text-xs text-gray-400 whitespace-nowrap">{conversation.time}</span>
            </div>
            
            <div className="flex justify-between mt-1">
              <p className="text-sm text-gray-400 truncate pr-2 blur-sm">{conversation.lastMessage}</p>
